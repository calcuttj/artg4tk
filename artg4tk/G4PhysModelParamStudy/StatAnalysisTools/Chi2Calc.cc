...
Chi2Calc::Chi2DataMC(const TH1* data, const TH1* mc, int& ndf, const double mcscale /* = 1. */)
{
  ...
  for (int id = x1Data; id <= x2Data; ++id) {
    ...
    //
    if (tmpData->GetBinContent(id) < 1.e-10 || tmpData->GetBinError(id) < 1.e-10)
      continue;
    //
...
double
Chi2Calc::Chi2DataMC(const TH1* data,
                     const TH1* mc,
                     int& ndf,
                     TMatrixD& covmtxdata,
                     const double mcscale /* = 1. */)
{
  ...
  for (int i = 0; i < nbins; ++i) {
    if (fabs(data->GetBinError(i + 1)) < 1.e-10)
      break;
    nvalid++;
  }
...
double
Chi2Calc::Chi2DataMC(const TGraphErrors* data,
                     const TH1* mc,
                     int& ndf,
                     const double mcscale /* = 1. */)
{
