...
void
artg4tk::RooMUHistSummary::endJob()
{
  ...
  for (; itr != fBand2Dirs.end(); ++itr) {
    for (unsigned int i = 0; i < (itr->second.size()); ++i) {
      if (!hfile.GetDirectory((itr->second)[i].c_str())) {
        fLogInfo << " Can NOT access specified directory for a variant " << (itr->second)[i]
                 << ", bail out";
        return;
      }
    }
  ...
  for (; itr != fBand2Dirs.end(); ++itr) {
    for (unsigned int i = 0; i < (itr->second.size()); ++i) {
      ...
      //
      if (!(*btconf == *tmpconf)) {
        fLogInfo << "Imcompatible beam-momentum-target configuration in directory "
                 << (itr->second)[i];
        return;
      }
    }
  ...
  BeamThinTargetConfig* outconf = tfs->makeAndRegister<BeamThinTargetConfig>(
    "Beam_ThinTarget_Config", "Beam-Momentum-TargetNucleus", *btconf);
  ...
  while (obj) {
    ...
    // for pair<string,PlotUtils::MULatErrorBand*>, pair<string,TMatrixT<double>*>,
    // pair<string,TH1D*> muh1->Write();
...
//
void
artg4tk::RooMUHistSummary::analyze(art::Event const& e)
{

  if (fVerbosity > 0)
    std::cout << "RooMUSummary: at event " << e.id().event() << "\n" << std::endl;

  return;
}

TH1*
artg4tk::RooMUHistSummary::matchExpSpectrum2MC(const int& secid,
