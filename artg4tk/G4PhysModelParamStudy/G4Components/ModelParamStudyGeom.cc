...
G4Material*
ModelParamStudyGeom::ResetMaterial(G4String mat)
{
...
void
ModelParamStudyGeom::ResetMaterial(G4Material* mat)
{
...
G4VPhysicalVolume*
ModelParamStudyGeom::ConstructTarget()
{
  ...
  */
  ...
  if (fShape != "G4Box" && fShape != "G4Tubs") {
    throw cet::exception("DetectorHolderService")
      << " For now, only G4Box or G4Tubs is supported; your shape = " << fShape << "\n";
  }
  ...
  if (!solid) {
    throw cet::exception("DetectorHolderService")
      << "Can NOT construct target (solid); check shape and/or size\n";
  }
  ...
  fPhysTarget = new G4PVPlacement(0,
                                  G4ThreeVector(fTargetPos[0], fTargetPos[1], fTargetPos[2]),
                                  "target_phys",
                                  fLogTarget,
...
G4double
ModelParamStudyGeom::GetRMax() const
{
