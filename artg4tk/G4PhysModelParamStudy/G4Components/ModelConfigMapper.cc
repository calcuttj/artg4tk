...
void
ModelConfigMapper::PrintCurrentSettings(std::string model)
{

  std::string mod = model;
  mod = ToLower(mod);

  if (mod == "bertini" || mod == "all")
    PrintBertiniSettings();
  if (mod == "inclxx" || mod == "all")
    PrintINCLXXSettings();
  if (mod == "precompound" || mod == "all")
...
void
ModelConfigMapper::SetVerbosity(const std::string& model, const bool& verb)
{
...
void
ModelConfigMapper::ChangeParameter(const std::string& model,
                                   ...
                                   bool verb)
{
  ...
  if (G4RunManager::GetRunManager()) // check if it exists at all
  {
    if (G4RunManager::GetRunManager()->GetUserPhysicsList()) {
      ...
      G4cout
        << " It is useless to change any model parameters since the changes will not propagate "
        << G4endl;
      return;
    }
  }
...
void
ModelConfigMapper::ChangeParameterByRatio(const std::string& model,
                                          const std::string& param,
                                          const double& ratio,
                                          bool verb)
{
  ...
  if (itr1 == (itr->second).end()) {
    // bail out with a warning
    //
    G4cout << " Parameter " << param << "(" << par << ") for model " << model << " NOT found "
           << G4endl;
    return;
  }
  ...
  if (itr3 == (itr2->second).end()) {
    std::cout << " can NOT find parameter " << itr1->second << " in the list of defaults"
              << std::endl;
  }
...
void
ModelConfigMapper::RestoreAllDefaults()
{
...
void
ModelConfigMapper::RestoreDefaults(const std::string& model)
{
...
std::string
ModelConfigMapper::ToLower(std::string input)
{
...
void
ModelConfigMapper::FillBertiniDefaults()
{

  fDEFAULTS.insert(std::pair<std::string, std::map<std::string, std::string>>(
    "bertini", std::map<std::string, std::string>()));

  std::map<std::string, std::map<std::string, std::string>>::iterator itr2 =
    fDEFAULTS.find("bertini");
  ...
  // but in practice in 9.6 they can only be changed via env.variables, due to some implementation
  // details
  //
  /*
     cmd << G4CascadeParameters::useTwoParam();
     // --> (itr2->second).insert(
     std::pair<std::string,std::string>("/useTwoParamNuclearRadius",cmd.str()) );
  ...
  */
  ...
  cmd << (G4CascadeParameters::radiusSmall() /
          G4CascadeParameters::radiusScale()); // due to specifics of Bertini implementation
  ...
  // !!!   (itr2->second).insert( std::pair<std::string,std::string>("/useBestNuclearModel","false")
  // ); // no corresponding access method; unclear D !!! From G4CascadeParameters.cc: BEST_PAR =
  // (0!=G4NUCMODEL_USE_BEST); probably means that if env.var. is NOT set, this option isn't in use

  return;
}

void
ModelConfigMapper::FillINCLXXDefaults()
{
...
void
ModelConfigMapper::FillPreCompoundDefaults()
{
  ...
  fDEFAULTS.insert(std::pair<std::string, std::map<std::string, std::string>>(
    "precompound", std::map<std::string, std::string>()));

  std::map<std::string, std::map<std::string, std::string>>::iterator itr2 =
    fDEFAULTS.find("precompound");
  ...
  // cmd << precoparams->GetLevelDensity()*CLHEP::MeV; // because the "native" default is
  // 0.10/CLHEP::MeV
  ...
  // cmd << precoparams->GetTransitionsR0()/CLHEP::fermi; // because the "native" default is
  // 0.6*CLHEP::fermi
  ...
  // cmd << precoparams->GetFermiEnergy()/CLHEP::MeV; // because the "native" default
  // is 35.0*CLHEP::MeV
  ...
  // cmd << precoparams->GetPrecoLowEnergy()/CLHEP::MeV; // because the "native" default is
  // 0.1*CLHEP::MeV
  ...
  // cmd << precoparams->GetMinExcitation()/CLHEP::eV; // because the "native" default is
  // 10*CLHEP::eV
  ...
  // cmd << precoparams->GetMaxLifeTime()/CLHEP::second; // because the "native" default is
  // 1000*CLHEP::second
  ...
  // cmd << precoparams->GetMinExPerNucleounForMF()/CLHEP::GeV; // because the "native" default is
  // 100*CLHEP::GeV
...
void
ModelConfigMapper::FillFTFPDefaults()
{
  ...
  fDEFAULTS.insert(std::pair<std::string, std::map<std::string, std::string>>(
    "ftfp", std::map<std::string, std::string>()));
  ...
  Projectile diffraction
           // SetParams( 3, 6.0/Xinel, 0.0 ,-6.0/Xinel*16.28, 3.0 , 0.0, 0.0  ,     0.93);// Target
  diffraction
           //
  ...
  cmd << bpar;
  (itr2->second)
    .insert(std::pair<std::string, std::string>("use_baryon_diff_disso_proj", cmd.str()));
  cmd.str("");
  cmd.clear();
  HDP.GetDefault("FTF_BARYON_DIFF_DISSO_TGT", bpar);
  cmd << bpar;
  (itr2->second)
    .insert(std::pair<std::string, std::string>("use_baryon_diff_disso_tgt", cmd.str()));
  cmd.str("");
  cmd.clear();
  ...
  HDP.GetDefault("FTF_BARYON_DIFF_M_PROJ",
                 dpar); // units of CLHEP::GeV will apply in the Set<...> method of G4FTFParameters
  cmd << dpar;
  ...
  HDP.GetDefault("FTF_BARYON_AVRG_PT2",
                 dpar); // units of (CLHEP::GeV)**2 will apply in Set<...> method of G4FTFParameters
  cmd << dpar;
  ...
  (itr2->second)
    .insert(std::pair<std::string, std::string>("use_nucdestr_p1_nbrn_proj", cmd.str()));
  cmd.str("");
  cmd.clear();
  ...
  cmd << dpar;
  (itr2->second)
    .insert(std::pair<std::string, std::string>("baryon_exci_e_per_wndnucln", cmd.str()));
  cmd.str("");
  cmd.clear();
...
void
ModelConfigMapper::FillConfigParamMapBertini()
{

  fConfigParameters.insert(std::pair<std::string, std::map<std::string, std::string>>(
    "bertini", std::map<std::string, std::string>()));

  std::map<std::string, std::map<std::string, std::string>>::iterator itr =
    fConfigParameters.find("bertini");
  ...
  // ---> (itr->second).insert(
  // std::pair<std::string,std::string>("use2paramnucradius","/useTwoParamNuclearRadius") );
  (itr->second)
    .insert(std::pair<std::string, std::string>("smallnucradius", "/smallNucleusRadius"));
  (itr->second)
    .insert(std::pair<std::string, std::string>("alpharadiusscale", "/alphaRadiusScale"));
...
void
ModelConfigMapper::FillConfigParamMapPreCo()
{
  ...
  std::map<std::string, std::map<std::string, std::string>>::iterator itr =
    fConfigParameters.find("precompound");
  ...
  (itr->second)
    .insert(std::pair<std::string, std::string>("fermienergy", "FermiEnergy")); // WAS MISSING !!!
  ...
  (itr->second)
    .insert(std::pair<std::string, std::string>("minexpernucleonformf", "MinExPerNucleonForMF"));
...
void
ModelConfigMapper::FillConfigParamMapFTFP()
{
  ...
  std::pair<std::string,std::string>("baryon_proc0_atop","BARYON_PROC0_ATOP") );
     (itr->second).insert(
  std::pair<std::string,std::string>("baryon_proc0_ymin","BARYON_PROC0_YMIN") );
  ...
  std::pair<std::string,std::string>("baryon_proc1_atop","BARYON_PROC1_ATOP") );
     (itr->second).insert(
  std::pair<std::string,std::string>("baryon_proc1_ymin","BARYON_PROC1_YMIN") );
  ...
  (itr->second)
    .insert(
      std::pair<std::string, std::string>("use_baryon_diff_disso_proj", "BARYON_DIFF_DISSO_PROJ"));
  (itr->second)
    .insert(
      std::pair<std::string, std::string>("use_baryon_diff_disso_tgt", "BARYON_DIFF_DISSO_TGT"));
  ...
  std::pair<std::string,std::string>("baryon_proc4_atop","BARYON_PROC4_ATOP") );
     (itr->second).insert(
  std::pair<std::string,std::string>("baryon_proc4_ymin","BARYON_PROC4_YMIN") ); RELEASE LATER !!!
*/
  ...
  (itr->second)
    .insert(
      std::pair<std::string, std::string>("baryon_delta_prob_qexchg", "BARYON_DELTA_PROB_QEXCHG"));
  (itr->second)
    .insert(
      std::pair<std::string, std::string>("baryon_prob_same_qexchg", "BARYON_PROB_SAME_QEXCHG"));
  (itr->second)
    .insert(std::pair<std::string, std::string>("baryon_diff_m_proj", "BARYON_DIFF_M_PROJ"));
  (itr->second)
    .insert(std::pair<std::string, std::string>("baryon_nondiff_m_proj", "BARYON_NONDIFF_M_PROJ"));
  (itr->second)
    .insert(std::pair<std::string, std::string>("baryon_diff_m_tgt", "BARYON_DIFF_M_TGT"));
  (itr->second)
    .insert(std::pair<std::string, std::string>("baryon_nondiff_m_tgt", "BARYON_NONDIFF_M_TGT"));
  ...
  (itr->second)
    .insert(
      std::pair<std::string, std::string>("use_nucdestr_p1_nbrn_proj", "NUCDESTR_P1_NBRN_PROJ"));
  (itr->second).insert(std::pair<std::string, std::string>("nucdestr_p1_tgt", "NUCDESTR_P1_TGT"));
  (itr->second)
    .insert(
      std::pair<std::string, std::string>("use_nucdestr_p1_adep_tgt", "NUCDESTR_P1_ADEP_TGT"));
  ...
  (itr->second)
    .insert(std::pair<std::string, std::string>("baryon_nucdestr_r2", "BARYON_NUCDESTR_R2"));
  (itr->second)
    .insert(std::pair<std::string, std::string>("baryon_exci_e_per_wndnucln",
                                                "BARYON_EXCI_E_PER_WNDNUCLN"));
  (itr->second)
    .insert(std::pair<std::string, std::string>("baryon_nucdestr_dof", "BARYON_NUCDESTR_DOF"));

  return;
}

void
ModelConfigMapper::PrintBertiniSettings()
{
  ...
  G4cout << fBaseCommand << "cascade/doCoalescence " << G4CascadeParameters::doCoalescence()
         << G4endl;

  // these params/methods are NOT available in G4.9.6-series, but only starting 10.1-ref03
  //
  G4cout << fBaseCommand << "cascade/piNAbsorption " << G4CascadeParameters::piNAbsorption()
         << G4endl;

  G4cout << fBaseCommand << "cascade/use3BodyMom " << G4CascadeParameters::use3BodyMom() << G4endl;

  G4cout << fBaseCommand << "cascade/usePhaseSpace " << G4CascadeParameters::usePhaseSpace()
         << G4endl;

  // technically speaking, these parameters are available in 9.6-series, together with their G4UI,
  // but in practice they can only be changed via env.variables, due to some implementation details
  //
  // ---> G4cout << fBaseCommand << "cascade/useTwoParamNuclearRadius " <<
  // G4CascadeParameters::useTwoParam() << G4endl;

  G4cout << fBaseCommand << "cascade/nuclearRadiusScale " << G4CascadeParameters::radiusScale()
         << G4endl;

  G4cout << fBaseCommand << "cascade/smallNuclearRadius "
         << (G4CascadeParameters::radiusSmall() / G4CascadeParameters::radiusScale())
         << "*radiusScale(" << G4CascadeParameters::radiusScale()
         << ") = " << G4CascadeParameters::radiusSmall() << G4endl;

  G4cout << fBaseCommand << "cascade/alphaRadiusScale " << G4CascadeParameters::radiusAlpha()
         << G4endl;

  G4cout << fBaseCommand << "cascade/shadowingRadius "
         << (G4CascadeParameters::radiusTrailing() / G4CascadeParameters::radiusScale())
         << "*radiusScale(" << G4CascadeParameters::radiusScale()
         << ") = " << G4CascadeParameters::radiusTrailing() << G4endl;

  G4cout << fBaseCommand << "cascade/fermiScale "
         << (G4CascadeParameters::fermiScale() / G4CascadeParameters::radiusScale())
         << "*radiusScale(" << G4CascadeParameters::radiusScale()
         << ") = " << G4CascadeParameters::fermiScale() << G4endl;

  G4cout << fBaseCommand << "cascade/crossSectionScale " << G4CascadeParameters::xsecScale()
         << G4endl;

  G4cout << fBaseCommand << "cascade/gammaQuasiDeutScale " << G4CascadeParameters::gammaQDScale()
         << G4endl;

  G4cout << fBaseCommand << "cascade/cluster2DPmax " << G4CascadeParameters::dpMaxDoublet()
         << G4endl;

  G4cout << fBaseCommand << "cascade/cluster3DPmax " << G4CascadeParameters::dpMaxTriplet()
         << G4endl;

  G4cout << fBaseCommand << "cascade/cluster4DPmax " << G4CascadeParameters::dpMaxAlpha() << G4endl;

  G4cout << fBaseCommand << "cascade/usePreCompound " << G4CascadeParameters::usePreCompound()
         << G4endl; // false/0 by default

  // --> !!!   (itr2->second).insert(
  // std::pair<std::string,std::string>("/useBestNuclearModel","false") ); // no corresponding
  // access method; unclear D !!! From G4CascadeParameters.cc: BEST_PAR = (0!=G4NUCMODEL_USE_BEST);
  // probably means that if env.var. is NOT set, this option isn't in use
  // --> !!!   (itr2->second).insert( std::pair<std::string,std::string>("/useBestNuclearModel","0")
  // ); // no corresponding access method; unclear D !!!
...
void
ModelConfigMapper::PrintINCLXXSettings()
{
...
void
ModelConfigMapper::PrintPreCompoundSettings()
{
  ...
  G4cout << "MinExPerNucleounForMF " << precoparams->GetMinExPerNucleounForMF() << " (in GeV)"
         << G4endl;
  ...
  // --> FIXME !! What is this ?? --> G4cout << "UseFilesNEW " << precoparams->UseFilesNEW() <<
  // G4endl;
  G4cout << "CorrelatedGamma " << precoparams->CorrelatedGamma() << G4endl;
  // --> FIXME !! What is this ?? --> G4cout << "StoreAllLevels " << precoparams->StoreAllLevels()
  // << G4endl; Figure out how to handle --> G4cout << "DedxChannelsType " <<
  // precoparams->GetDeexChannelsType() << G4endl;

  return;
}

void
ModelConfigMapper::PrintFTFPSettings()
{
...
void
ModelConfigMapper::ChangeParameterViaG4UI(const std::string& model,
                                          const std::string& param,
                                          const double& value,
                                          bool verb)
{
  ...
  //      G4cout << " Cross-check  usePreCompound = " << G4CascadeParameters::usePreCompound() <<
  //      G4endl; G4cout << " Cross-check radiusScale = " << G4CascadeParameters::radiusScale() <<
  //      G4endl; G4cout << " Cross-check xsecScale = = " << G4CascadeParameters::xsecScale() <<
  //      G4endl;
...
void
ModelConfigMapper::ChangeParameterPreCo(const std::string& param, const double& value, bool verb)
{
...
void
ModelConfigMapper::ChangeParameterFTFP(const std::string& pname, const double& value, bool verb)
{
