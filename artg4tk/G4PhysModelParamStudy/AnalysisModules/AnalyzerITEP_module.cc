...
namespace artg4tk {
  ...
  private:
    ...
    //
    double fDeltaTheta;
    std::vector<double> /* fKECBin, emin, emax, */ fThetaCBin, fCosThetaMin, fCosThetaMax,
      fDeltaCosTheta;
...
artg4tk::AnalyzerITEP::AnalyzerITEP(const fhicl::ParameterSet& p)
  : artg4tk::ModelParamAnalyzerBase(p), fChi2Calc(0)
...
artg4tk::AnalyzerITEP::beginJob()
{
  ...
  */
  ...
  for (size_t ith = 0; ith < fThetaCBin.size(); ++ith) {
    ...
    name += "deg";
    fProtonKE.push_back(
      tfs->make<TH1D>(name.c_str(), title.c_str(), fNumProtonKEBins, fProtonKEBins));

    name = "neutron_at_";
    name += tmp.str();
    name += "deg";

    fNeutronKE.push_back(
      tfs->make<TH1D>(name.c_str(), title.c_str(), fNumNeutronKEBins, fNeutronKEBins));
  }
...
void
artg4tk::AnalyzerITEP::analyze(const art::Event& e)
{
...
TH1*
artg4tk::AnalyzerITEP::matchExpSpectrum2MC(const int& secid,
...
std::string
artg4tk::AnalyzerITEP::extractThetaFromTitle(const std::string& title)
{
  ...
  pos1 = ret.find(".");
  if (ret.compare(pos1 + 1, 1, "0") ==
      0) // a bit "flaky" since there may something like "1.02" but it'll be redone anyway
  {
...
void
artg4tk::AnalyzerITEP::calculateChi2()
{
