...
artg4tk::AnalyzerHARP::beginJob()
{
  ...
  // NOTE: use of std::to_string(double) results in something like this: 0.1 --> "0.100000" (too
  // much...)
  ...
  for (int i = 0; i < fNThetaBinsFW; i++) {
    ...
    theta_bin_fw += " [rad]";
    //      theta_bin_fw = std::to_string(thetaMin) + "<theta<" + std::to_string(thetaMax) +
    //      "[rad]";

    hname = "piminus_FW_" + std::to_string(i);
    fHistoSecPiMinusFW.push_back(
      tfs->make<TH1D>(hname.c_str(), theta_bin_fw.c_str(), nparbins_fw, parbins_fw));

    hname = "piplus_FW_" + std::to_string(i);
    fHistoSecPiPlusFW.push_back(
      tfs->make<TH1D>(hname.c_str(), theta_bin_fw.c_str(), nparbins_fw, parbins_fw));

    hname = "proton_FW_" + std::to_string(i);
    fHistoSecProtonFW.push_back(
      tfs->make<TH1D>(hname.c_str(), theta_bin_fw.c_str(), nparbins_fw, parbins_fw));
  }
  ...
  for (int i = 0; i < fNThetaBinsLA; i++) {
    ...
    fHistoSecPiMinusLA.push_back(
      tfs->make<TH1D>(hname.c_str(), theta_bin_la.c_str(), nparbins_la, parbins_la));

    hname = "piplus_LA_" + std::to_string(i);
    fHistoSecPiPlusLA.push_back(
      tfs->make<TH1D>(hname.c_str(), theta_bin_la.c_str(), nparbins_la, parbins_la));

    hname = "proton_LA_" + std::to_string(i);
    fHistoSecProtonLA.push_back(
      tfs->make<TH1D>(hname.c_str(), theta_bin_la.c_str(), nparbins_la, parbins_la));
  }
...
void
artg4tk::AnalyzerHARP::analyze(const art::Event& e)
{
  ...
  for (int ip = 0; ip < nsec; ++ip) {
    ...
    if (theta < fThetaMinLA)
      continue;
    if (theta > fThetaMinLA + fDeltaThetaLA * fNThetaBinsLA)
      continue;
    int itheta = (theta - fThetaMinLA) / fDeltaThetaLA;
    if (itheta < 0 || itheta >= fNThetaBinsLA)
      continue;
...
TH1*
artg4tk::AnalyzerHARP::matchExpSpectrum2MC(const int& secid,
...
artg4tk::AnalyzerHARP::extractThetaBinFromTitle(const std::string& title)
{
...
void
artg4tk::AnalyzerHARP::calculateChi2()
{
...
DEFINE_ART_MODULE(artg4tk::AnalyzerHARP)
