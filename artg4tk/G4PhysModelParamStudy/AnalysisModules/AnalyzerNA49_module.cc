...
artg4tk::AnalyzerNA49::beginJob()
{
  ...
  fHistoNSec =
    tfs->make<TH1D>("NSec", "Number of secondary per inelastic interaction", 100, 0., 100);
  ...
  // fHistoSecProton.push_back( tfs->make<TH1D>( "proton_dNdxF",   "Integrated", fNPBinsXF-1,
  // fPBinsXF ) ); fHistoSecProton.push_back( tfs->make<TProfile>( "proton_pT",  "Integrated",
  // fNPBinsXF-1, fPBinsXF, 0., 10. ) );
                                             ...
                                             // while all other are N+1 !!!
  {
    ...
    */
    ...
    // ---> fHistoPTProton.push_back( tfs->make<TH1D>( hname.c_str(), title.c_str(), fNPBinsPT-1,
    // fPBinsPT ) );
  ...
  // npbarbins, pbarbins ) );
  // ---> fHistoSecAntiProton.push_back( tfs->make<TProfile>( "antiproton_pT", "Integrated",
  // npbarbins, pbarbins, 0., 10. ) );
  fHistoSecAntiProton.push_back(new TH1D("tmp_antiproton_dNdxF", "Integrated", 2000, -1., 1.));
  fHistoSecAntiProton.push_back(
    new TProfile("tmp_antiproton_pT", "Integrated", 2000, -1., 1., 0., 10.));
  ...
  // ---> fHistoSecPiMinus.push_back( tfs->make<TH1D>( "piminus_dNdxF",   "Integrated",
  // fNPiBinsXF-1, fPiBinsXF ) );
  // ---> fHistoSecPiMinus.push_back( tfs->make<TProfile>( "piminus_pT",  "Integrated",
  // fNPiBinsXF-1, fPiBinsXF, 0., 10. ) );
  ...
  // ---> fHistoSecPiPlus.push_back( tfs->make<TH1D>( "piplus_dNdxF",   "Integrated", fNPiBinsXF-1,
  // fPiBinsXF ) );
  // ---> fHistoSecPiPlus.push_back( tfs->make<TProfile>( "piplus_pT",  "Integrated", fNPiBinsXF-1,
  // fPiBinsXF, 0., 10. ) );
  ...
  for (int nb = 0; nb < fNPiBinsXF - 1; ++nb) {
    ...
    */
    ...
    // ---> fHistoPTPiMinus.push_back( tfs->make<TH1D>( hname.c_str(), title.c_str(), fNPiBinsPT-1,
    // fPiBinsPT ) );
    fHistoPTPiMinus.push_back(new TH1D(hname.c_str(), title.c_str(), 250, 0., 2.5));
    hname = "tmp_piplus_pT_DDIFF_" + osBin.str();
    // ---> fHistoPTPiPlus.push_back( tfs->make<TH1D>( hname.c_str(),  title.c_str(), fNPiBinsPT-1,
    // fPiBinsPT ) );
  ...
  // fHistoSecNeutron.push_back( tfs->make<TH1D>( "tmp_neutron_dNdxF",    "Integrated", 10,
  // 0.05, 1.05 ) );
...
artg4tk::AnalyzerNA49::endJob()
{
  ...
  if (fIncludeExpData) {
    bool ok =
      matchVDBRec2MC(fBTConf.GetBeamPartID(), fBTConf.GetBeamMomentum(), fBTConf.GetTargetID());
    if (!ok) {
    ...
    for (size_t ih = 0; ih < fHistoSecProton.size(); ++ih) {
      for (itr = fVDBRecID2MC.begin(); itr != fVDBRecID2MC.end(); ++itr) {
        if (fHistoSecProton[ih] == itr->second)
          break;
      }
    ...
    for (size_t ium = 0; ium < unmatched.size(); ++ium) {
      ...
      if (!(classname == "TProfile"))
        h1->Scale(1. / ((double)stat), "width");
    }
    unmatched.clear();
    for (size_t ih = 0; ih < fHistoPTProton.size(); ++ih) {
      for (itr = fVDBRecID2MC.begin(); itr != fVDBRecID2MC.end(); ++itr) {
        if (fHistoPTProton[ih] == itr->second)
          break;
      }
      if (itr == fVDBRecID2MC.end())
        unmatched.push_back(ih);
    ...
    for (size_t ium = 0; ium < unmatched.size(); ++ium) {
      ...
      h1->Scale(
        fXSecOnTarget /
        ((double)stat)); // Note: NO scaling with "width" here
                         // because it had to be E/dP3 - part of it is already taken into account as
                         // weight (analysis) and the dPT2 contribution is done just above
    }
    ...
    for (size_t ih = 0; ih < fHistoSecAntiProton.size(); ++ih) {
      for (itr = fVDBRecID2MC.begin(); itr != fVDBRecID2MC.end(); ++itr) {
        if (fHistoSecAntiProton[ih] == itr->second)
          break;
      }
      if (itr == fVDBRecID2MC.end())
        unmatched.push_back(ih);
    ...
    for (size_t ih = 0; ih < fHistoSecPiMinus.size(); ++ih) {
      for (itr = fVDBRecID2MC.begin(); itr != fVDBRecID2MC.end(); ++itr) {
        if (fHistoSecPiMinus[ih] == itr->second)
          break;
      }
      if (itr == fVDBRecID2MC.end())
        unmatched.push_back(ih);
    ...
    for (size_t ih = 0; ih < fHistoPTPiMinus.size(); ++ih) {
      for (itr = fVDBRecID2MC.begin(); itr != fVDBRecID2MC.end(); ++itr) {
        if (fHistoPTPiMinus[ih] == itr->second)
          break;
      }
      if (itr == fVDBRecID2MC.end())
        unmatched.push_back(ih);
    ...
    for (size_t ium = 0; ium < unmatched.size(); ++ium) {
      ...
      h1->Scale(fXSecOnTarget /
                ((double)stat)); // Note: NO scaling with "width" here - see earlier comment
    }
    ...
    for (size_t ih = 0; ih < fHistoSecPiPlus.size(); ++ih) {
      for (itr = fVDBRecID2MC.begin(); itr != fVDBRecID2MC.end(); ++itr) {
        if (fHistoSecPiPlus[ih] == itr->second)
          break;
      }
      if (itr == fVDBRecID2MC.end())
        unmatched.push_back(ih);
    ...
    for (size_t ih = 0; ih < fHistoPTPiPlus.size(); ++ih) {
      for (itr = fVDBRecID2MC.begin(); itr != fVDBRecID2MC.end(); ++itr) {
        if (fHistoPTPiPlus[ih] == itr->second)
          break;
      }
      if (itr == fVDBRecID2MC.end())
        unmatched.push_back(ih);
    ...
    for (size_t ium = 0; ium < unmatched.size(); ++ium) {
      ...
      h1->Scale(fXSecOnTarget /
                ((double)stat)); // Note: NO scaling with "width" here - see comment earlier
    }

    rebinMC2Data("tmp_");
    for (itr = fVDBRecID2MC.begin(); itr != fVDBRecID2MC.end(); ++itr) {
      if (!itr->second)
        continue; // // NULL histo for this exp.data record (some might be skipped)
      std::string hname = (itr->second)->GetName();
      if (hname.find("_pT") == std::string::npos)
        (itr->second)->Scale(1. / ((double)stat), "width");
  ...
  } else // no exp.data included in the analysis
  {
    ...
    for (size_t i = 0; i < fHistoPTProton.size(); ++i) {
      ...
      h->Scale(fXSecOnTarget /
               ((double)stat)); // Note: NO scaling with "width" here
                                // because it had to be E/dP3 - part of it is already taken into
                                // account as weight and the dPT2 contribution is done below
      // fHistoPTProton[i]->Scale( fXSecOnTarget/((double)stat) ); // Note: NO scaling with "width"
      // here because it had to be E/dP3 - part of it is already taken into account as weight and
      // the dPT2 contribution is done below
    }
    //
    ...
    for (size_t i = 0; i < fHistoPTPiMinus.size(); ++i) {
      ...
      // fHistoPTPiMinus[i]->Scale( fXSecOnTarget/((double)stat) ); // Note: NO scaling with "width"
      // because it had to be E/dP3
    ...
    for (size_t i = 0; i < fHistoPTPiPlus.size(); ++i) {
      ...
      h->Scale(fXSecOnTarget / ((double)stat));
      // fHistoPTPiPlus[i]->Scale( fXSecOnTarget/((double)stat) ); // NO scaling with "width" - see
      // earlier comment for fHistoPTPiMinus
    }
...
void
artg4tk::AnalyzerNA49::analyze(const art::Event& e)
{
  ...
  if (!fXSecInit)
    initXSecOnTarget(firstint->GetMaterialName(), firstint->GetIncoming());

  if (!ensureBeamTgtConfig(e))
    return;
  ...
  */
  ...
  CLHEP::HepLorentzVector labp(
    0., 0., std::sqrt(etot * (etot + 2. * mass)), (etot + mass + 0.93827));
  ...
  for (int ip = 0; ip < nsec; ++ip) {
    ...
    if (pname == "neutron") {
      ...
      int nb = -1;
      for (int ib = 0; ib < fNPBinsXF; ++ib) // # number of proton bins are N and the size is N+1,
                                             // while for pions # is N+1 need to make it uniform !!!
      {
...
TH1*
artg4tk::AnalyzerNA49::matchExpSpectrum2MC(const int& secid,
...
double
artg4tk::AnalyzerNA49::calculateBinWeight(const CLHEP::HepLorentzVector& labp,
                                          const std::string& pname,
                                          double Ekin,
                                          double mass,
                                          double pT,
                                          ...
                                          double sqrt_s)
{
  ...
  */
  ...
  //
  //
  // ---> double dpT2  = 0.; // fPiBinsPT[pTbin+1]*fPiBinsPT[pTbin+1] -
  // fPiBinsPT[pTbin]*fPiBinsPT[pTbin];
...
void
artg4tk::AnalyzerNA49::calculateChi2()
{
  ...
  for (; itr != fVDBRecID2MC.end(); ++itr) {
    ...
    if (hname.find(secondary) == std::string::npos ||
        hname.find(secondary) != 0) // change of histo name
    {
...
std::string
artg4tk::AnalyzerNA49::extractXFBinFromTitle(const std::string& title)
{
