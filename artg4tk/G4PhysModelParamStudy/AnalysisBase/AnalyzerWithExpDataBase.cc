...
artg4tk::AnalyzerWithExpDataBase::AnalyzerWithExpDataBase(const fhicl::ParameterSet& p)
  : art::EDAnalyzer(p), fLogInfo("AnalyzerExpData")
{
  ...
  if (p.has_key("IncludeExpData")) {
    ...
    fVDBRecordID =
      (p.get<fhicl::ParameterSet>("IncludeExpData")).get<std::vector<int>>("DBRecords");
...
void
artg4tk::AnalyzerWithExpDataBase::endJob()
{
...
bool
artg4tk::AnalyzerWithExpDataBase::matchVDBRec2MC(const int& bid, const double& bmom, const int& tgt)
{
  ...
  if (!ok) {
    fLogInfo << " No exp.data to match beam particle = " << bid << " of momentum = " << bmom
             << " and target id = " << tgt;
    return ok;
  }
  ...
  for (size_t i = 0; i < fVDBRecID2MC.size(); ++i) {
    ...
    //
    TH1* h = matchExpSpectrum2MC(
      fJSON2Data->GetMetaData().fSecondaryPID, cond, fJSON2Data->GetMetaData().fObservable);
    if (h) {
      fVDBRecID2MC[i].second = h;
      ok = true;
    } else {
      std::cout << " NO match for: " << fJSON2Data->GetMetaData().fTitle << " ---> histo = NULL "
                << std::endl;
    }
  }
...
bool
artg4tk::AnalyzerWithExpDataBase::findExpDataByBeamTarget(const int& bid,
                                                          const double& bmom,
                                                          const int& tgt)
{
...
void
artg4tk::AnalyzerWithExpDataBase::rebinMC2Data(const std::string& tag2rm)
{
  ...
  for (; itr != fVDBRecID2MC.end(); ++itr) {
    ...
    if (pos != std::string::npos)
      hname.erase(pos, std::string(tag2rm).length());
...
void
artg4tk::AnalyzerWithExpDataBase::overlayDataMC()
{
