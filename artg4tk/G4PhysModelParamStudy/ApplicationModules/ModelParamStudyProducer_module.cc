...
artg4tk::ModelParamStudyProducer::ModelParamStudyProducer(const fhicl::ParameterSet& p)
  : EDProducer{p}, fLogInfo("ModelParamStudyProducer")
{
  ...
  */
  ...
  if (!fDefaultPhysics) {
    ...
    if (!modelcfg.is_empty())
      physcfgservice->ConfigureModel("Bertini", modelcfg);
  ...
  if (fRandomSeed == -1) {
    ...
    fRandomSeed = ((fRandomSeed & 0xFFFF0000) >> 16) |
                  ((fRandomSeed & 0x0000FFFF) << 16); // exchange upper and lower word
...
//
void
artg4tk::ModelParamStudyProducer::beginJob()
{
...
//
void
artg4tk::ModelParamStudyProducer::beginRun(art::Run& run)
{
  ...
  /* testing/debugging stuff - will be removed shortly
     ...
     parser.Write( "feedback-geom.gdml",
                   ...
                   false ); // 3rd arg set to "false" means NOT to concatinate each volume name
                            // with its logical address in hexadecimal format (D=true, and the Read
     is set to strip this extention)
  */
  /*
...
void
artg4tk::ModelParamStudyProducer::produce(art::Event& e)
{
  ...
  for (GenParticleCollection::const_iterator i = primgenparts->begin(); i != primgenparts->end();
       ++i) {
    int pdgcode = i->pdgId();
    ...
    G4PrimaryVertex* g4vtx =
      new G4PrimaryVertex(i->position() * CLHEP::cm, 0.); // 4th arg is time(ns)
    //
    ...
    /*
          ...
          G4VPhysicalVolume*   pdaughter = world->GetDaughter(0); // "first daughter" which is
       subworld G4VSolid*            solid = pdaughter->GetLogicalVolume()->GetSolid(); double z =
       solid->DistanceToOut( G4ThreeVector(0.,0.,0.), G4ThreeVector(0.,0.,-1.) ); // NOTE that dist
       is always positive
          //
          if ( mom.z() > 0. ) // if the z-momentum is positive (left-to-right), then start at the
       left edge
          {
             z *= -1.;
          }
          //
          CLHEP::Hep3Vector pos( 237.5, 0., (pdaughter->GetTranslation().z()+z*0.999) ); // z is in
       mm !!! and shift it by 0.1% inward !!!
       ...
       CLHEP::Hep3Vector(23.75,0.,-40.54646)*CLHEP::cm, 0. );
          //
    */

    G4PrimaryParticle* g4prim = new G4PrimaryParticle(
      g4pd, mom.x() * CLHEP::GeV, mom.y() * CLHEP::GeV, mom.z() * CLHEP::GeV, mom.e() * CLHEP::GeV);
  ...
  if (fVerbosity > 0) {
    fLogInfo << "G4Event " << e.id().event()
             << " has been processed by EventManager "; // << std::endl;
  }
  fRM->AnalyzeEvent(fCurrentG4Event);
  // fRM->UpdateScoring(); // can NOT use it outside G4RunManager (or derived) since this method is
  // protected
...
//
void
artg4tk::ModelParamStudyProducer::endRun(art::Run&)
{
...
void
artg4tk::ModelParamStudyProducer::clear()
{
