...
// --> seems to come via some other inc #include
...
artg4tk::ProcLevelMPVaryProducer::ProcLevelMPVaryProducer(const fhicl::ParameterSet& p)
  : EDProducer{p}, fLogInfo("ProcLevelMPVaryProducer")
{
  ...
  */
  ...
  if (!fDefaultPhysics) {
    ...
    // mock up tmp pset to look like the one in phys.list-based producer for the
    // RunProduct::Fill(...)
    fhicl::ParameterSet tmp;
    tmp.put(fModelName, modelcfg);
    fModelConfig->Fill(tmp);
    if (!modelcfg.is_empty())
      physcfgservice->ConfigureModel(fModelName, modelcfg);
  }
  ...
  // << G4endl;
  // !!!      G4cout << " Cross-check radiusScale = " << G4CascadeParameters::radiusScale() <<
  // G4endl;
  ...
  // technically speaking, a product can be registered with an "instance name:
  // produces<prod>("instancename") but then it needs to be placed into an event as follows:
  // e.put(std::move(prod), "instancename")
  //
  produces<ArtG4tkVtx>();
}
...
//
void
artg4tk::ProcLevelMPVaryProducer::beginJob()
{
...
//
void
artg4tk::ProcLevelMPVaryProducer::beginRun(art::Run& run)
{

  // here do proc.level stuff/init ??? Or in the ctor ??? or maybe in the very fisrt event in case
  // !IsFullyInit ???
...
void
artg4tk::ProcLevelMPVaryProducer::produce(art::Event& e)
{
  ...
  if (primgenparts->size() > 1) {
    fLogInfo
      << " Multiple Generator Particles - only the 1st is taken as a PROJECTILE "; // << std::endl;
  }
  ...
  if (!fTrack || !fStep) {
    fLogInfo << " Event " << e.id().event()
             << ": projectile problem - INVALID track and/or step; bail out ";
    return;
  }
  ...
  //
  firstint->SetIncoming(
    ArtG4tkParticle(fTrack->GetParticleDefinition()->GetPDGEncoding(), fTrack->GetMomentum()));

  for (int is = 0; is < nsec; ++is) {
    G4Track* strk = thechange->GetSecondary(is);
    firstint->AddOutcoming(
      ArtG4tkParticle(strk->GetParticleDefinition()->GetPDGEncoding(), strk->GetMomentum()));
  }
...
//
void
artg4tk::ProcLevelMPVaryProducer::endRun(art::Run&)
{
...
void
artg4tk::ProcLevelMPVaryProducer::initParticles()
{
...
void
artg4tk::ProcLevelMPVaryProducer::initProcess()
{
  ...
  for (unsigned int i = 0; i < mod.size(); ++i)
    mod[i] = std::tolower(mod[i]);

  ProcessWrapper* pw = 0;

  if (mod.find("bertini") != std::string::npos) {
    pw = new ProcessWrapper(
      "BertiniProcessWrapper"); // I should probably give it a unique name
    ...
    inclxx->SetMaxEnergy(20. *
                         CLHEP::GeV); // this is what stands in the INCL-based PL for p,n,pi
...
void
artg4tk::ProcLevelMPVaryProducer::setProjectile(const GenParticle& genpart)
{
  ...
  //
  fTrack = new G4Track(
    new G4DynamicParticle(partDef, genpart.momentum() * CLHEP::GeV), // momentum in GeV/c !!!
    0.,
    genpart.position() * CLHEP::mm); // 2nd arg is time (in ns ?)
...
void
artg4tk::ProcLevelMPVaryProducer::resetProjectileKine(const GenParticle& part)
{
