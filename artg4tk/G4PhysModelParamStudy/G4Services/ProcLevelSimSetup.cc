...
void
artg4tk::ProcLevelSimSetupService::Init()
{
  ...
  //                                                fTgtMaterial, // fMaterial can be NULL at this
  //                                                point - no big deal though...
  //						"Box",0,0,0);
  G4PVPlacement* pFrame = new G4PVPlacement(
    0,
    G4ThreeVector(),
    "Box",
    new G4LogicalVolume(
      new G4Box("Box", 1000.0 * CLHEP::mm, 1000.0 * CLHEP::mm, 1000.0 * CLHEP::mm),
  ...
  std::ostringstream osmat(std::ios_base::out |
                           std::ios_base::app); // to enable appending in output operations
  osmat.clear();
  osmat.str("G4_");
