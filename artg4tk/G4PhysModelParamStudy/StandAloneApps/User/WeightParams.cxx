...
main(int argc, char** argv)
{
  ...
  /*
     double chi2thr = 2250.;
     if ( fname.find("piminus") != std::string::npos || fname.find("piplus") != std::string::npos )
     chi2thr = 4500.;
  */
  ...
  std::map<
    std::string,
    std::map<std::string,
             std::map<double, std::vector<std::pair<std::string, std::pair<double, double>>>>>>
    parvschi2;
  ...
  while (key) {
    ...
    for (int i = 0; i < objarr->GetEntries(); ++i) {
      ...
      if (pname.find("USE") != std::string::npos)
        continue; // skip ON/OFF switches
      std::string sval = str.substr(str.find("=") + 1);
      double val = std::stod(sval);
      std::map<std::string, std::map<std::string, std::vector<double>>>::iterator itr1 =
        mparams.find(model);
      if (itr1 == mparams.end()) {
        mparams.insert(std::pair<std::string, std::map<std::string, std::vector<double>>>(
          model, std::map<std::string, std::vector<double>>()));
        itr1 = mparams.find(model);
      }
      std::map<std::string, std::vector<double>>::iterator itr2 = (itr1->second).find(pname);
      if (itr2 == (itr1->second).end()) {
        (itr1->second)
          .insert(std::pair<std::string, std::vector<double>>(pname, std::vector<double>()));
      ...
      if (!pexists)
        (itr2->second).push_back(val);

      std::map<std::string,
               std::map<std::string,
                        std::map<double,
                                 std::vector<std::pair<std::string, std::pair<double, double>>>>>>::
        iterator ichi2_1 = parvschi2.find(model);
      ...
      std::map<std::string,
               std::map<double, std::vector<std::pair<std::string, std::pair<double, double>>>>>::
        iterator ichi2_2 = (ichi2_1->second).find(pname);
      ...
      if ((ichi2_2->second).find(val) != (ichi2_2->second).end()) {
        ...
        (ichi2_2->second)
          .insert(std::pair<double, std::vector<std::pair<std::string, std::pair<double, double>>>>(
            val, std::vector<std::pair<std::string, std::pair<double, double>>>()));
      }
    }
  ...
  for (; itr1 != mparams.end(); ++itr1) {
    ...
    for (; itr2 != (itr1->second).end(); ++itr2) {
      ...
      for (size_t i = 0; i < (itr2->second).size(); ++i) {
        if ((itr2->second)[i] < pmin)
          pmin = (itr2->second)[i];
        if ((itr2->second)[i] > pmax)
          pmax = (itr2->second)[i];
      }
      ...
      for (size_t i = 0; i < SecPartName.size(); ++i) {
        histos.insert(
          std::pair<std::string, std::vector<TProfile*>>(SecPartName[i], std::vector<TProfile*>()));
        std::map<std::string, std::vector<TProfile*>>::iterator ih = histos.find(SecPartName[i]);
        (ih->second)
          .push_back(new TProfile(
            (SecPartName[i] + "_Chi2_" + hname).c_str(), "", 25, hmin, hmax, 0., 50000.));
        (ih->second).back()->GetXaxis()->SetTitle((itr1->first + " " + itr2->first).c_str());
        (ih->second).back()->GetYaxis()->SetTitle(("Chi2"));
        (ih->second)
          .push_back(new TProfile(
            (SecPartName[i] + "_Chi2NDF_" + hname).c_str(), "", 25, hmin, hmax, 0., 50000.));
        (ih->second).back()->GetXaxis()->SetTitle((itr1->first + " " + itr2->first).c_str());
        (ih->second).back()->GetYaxis()->SetTitle(("Chi2/NDF"));
        (ih->second)
          .push_back(
            new TProfile((SecPartName[i] + "_" + hname).c_str(), "", 25, hmin, hmax, 0., 50000.));
        (ih->second).back()->GetXaxis()->SetTitle((itr1->first + " " + itr2->first).c_str());
        (ih->second).back()->GetYaxis()->SetTitle(("Number of entries"));
        (ih->second)
          .push_back(new TProfile(
            (SecPartName[i] + "_Wt_" + hname).c_str(), "", 25, hmin, hmax, 0., 50000.));
        (ih->second).back()->GetXaxis()->SetTitle((itr1->first + " " + itr2->first).c_str());
        (ih->second).back()->GetYaxis()->SetTitle(("exp(-0.5*chi2)"));
      }
      histos.insert(
        std::pair<std::string, std::vector<TProfile*>>("total", std::vector<TProfile*>()));
      std::map<std::string, std::vector<TProfile*>>::iterator ih = histos.find("total");
      (ih->second)
        .push_back(new TProfile(("total_" + hname).c_str(), "", 25, hmin, hmax, 0., 50000.));
      (ih->second).back()->GetXaxis()->SetTitle((itr1->first + " " + itr2->first).c_str());
      (ih->second).back()->GetYaxis()->SetTitle(("Number of entries"));
      (ih->second)
        .push_back(new TProfile(("total_Chi2_" + hname).c_str(), "", 25, hmin, hmax, 0., 50000.));
      (ih->second).back()->GetXaxis()->SetTitle((itr1->first + " " + itr2->first).c_str());
      (ih->second).back()->GetYaxis()->SetTitle(("Chi2"));
      (ih->second)
        .push_back(
          new TProfile(("total_Chi2NDF_" + hname).c_str(), "", 25, hmin, hmax, 0., 50000.));
      (ih->second).back()->GetXaxis()->SetTitle((itr1->first + " " + itr2->first).c_str());
      (ih->second).back()->GetYaxis()->SetTitle(("Chi2/NDF"));
      (ih->second)
        .push_back(new TProfile(("total_Wt_" + hname).c_str(), "", 25, hmin, hmax, 0., 50000.));
      (ih->second).back()->GetXaxis()->SetTitle((itr1->first + " " + itr2->first).c_str());
      (ih->second).back()->GetYaxis()->SetTitle(("exp(-0.5*chi2)"));
    }
  }
    ...
    for (int ipr = 0; ipr < objarr->GetEntries(); ++ipr) {
      ...
      for (size_t i1 = 0; i1 < SecPartName.size(); ++i1) {
        ...
        std::map<std::string,
                 std::map<double, std::vector<std::pair<std::string, std::pair<double, double>>>>>::
          iterator ichi2_2 = (ichi2_1->second).find(pname);
        std::map<double, std::vector<std::pair<std::string, std::pair<double, double>>>>::iterator
          ichi2_3 = (ichi2_2->second).find(pval);
  ...
  for (; itr != parvschi2.end(); ++itr) {
    std::map<std::string,
             std::map<double, std::vector<std::pair<std::string, std::pair<double, double>>>>>::
      iterator itr_1 = (itr->second).begin();
    for (; itr_1 != (itr->second).end(); ++itr_1) {
      ...
      for (; itr_2 != (itr_1->second).end(); ++itr_2) {
        ...
        if (itr_chi2min->second > totchi2)
          itr_chi2min->second = totchi2;
      }
    }
  }
  ...
  /*
     ...
     {
        // std::cout << " parameter = " << chi2min[i].first << " --> chi2min = " <<
     chi2min[i].second << std::endl; std::cout << " parameter = " << itr_chi2min->first << " -->
     chi2min = " << itr_chi2min->second << std::endl;
     }
  */
  ...
  for (itr = parvschi2.begin(); itr != parvschi2.end(); ++itr) {
    ...
    for (; itr_1 != (itr->second).end(); ++itr_1) {
      ...
      for (; itr_2 != (itr_1->second).end(); ++itr_2) {
        ...
        std::map<std::string, std::vector<TProfile*>>::iterator ih = histos.find("total");
        // std::cout << " pname = " << pname << " totchi2 = " << totchi2 << " minchi2 = " <<
        // chi2min[pname] << std::endl;
        for (size_t i3 = 0; i3 < (ih->second).size(); ++i3) {
          ...
          if (hname.find(model) != std::string::npos && hname.find(pname) != std::string::npos) {
            ...
            if (hname.find("_Chi2NDF_") != std::string::npos) {
              if (totndf > 0.)
                (ih->second)[i3]->Fill(pval, totchi2 / totndf);
              continue;
            } else if (hname.find("_Wt_") != std::string::npos) {
      ...
      if (pos != std::string::npos) {
        ...
        for (size_t i1 = 0; i1 < (ih->second).size(); ++i1) {
          if (hname == ((ih->second)[i1]->GetName())) {
            ...
            double scale = 1.;
            if (Integral_Wt > 1.e-307)
              scale = Integral / Integral_Wt;
