  ...
  for (unsigned int i = 0; i < flist.size(); ++i) {
    ...
    while (key) {
      ...
      for (; ip < particles.size(); ++ip) {
        if (hname.find(particles[ip].first) != std::string::npos)
          break;
      }
      ...
      for (size_t id = 0; id < pdefaults.size(); ++id) {
        if (hname.find(pdefaults[id].first) != std::string::npos) {
          ...
          TArrow ar(
            pdefaults[id].second, ymin, pdefaults[id].second, 0.5 * (ymin + ymax), 0.02, "<|");
          ar.SetLineWidth(3);
          ar.SetLineColor(7);
          pad1list[i]->cd();
          ar.Draw();
...
PlotParametersVsWt()
{
  ...
  for (unsigned int i = 0; i < flist.size(); ++i) {
    ...
    double ymax = 1.e+25;
    double ymin = 1.e-150;
    if (flist[i].find("piminus") != std::string::npos ||
        flist[i].find("piplus") != std::string::npos)
      ymin = 1.e-250;
