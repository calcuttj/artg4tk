...
void DrawVariantsHARP(const int, std::string*, std::string, std::string, int*, double&);
void VariantsHARP2Legend(const int nv,
                         std::string* vdirs,
                         std::string,
                         std::string rname,
                         std::string* vlabels,
...
main(int argc, char** argv)
{
  ...
  std::string rsclabel[NRsc] = {
    "RadiusScale=3.5", "RadiusScale=2.1", "RadiusScale=1.4", "RadiusScale=1."};
  int colrsc[NRsc] = {kBlue, kGreen, kMagenta, kCyan}; // kCyan=7
  const int NXSsc = 6;
  std::string dxssc[NXSsc] = {"XSsc2", "XSsc15", "XSsc125", "XSsc075", "XSsc05", "XSsc01"};
  std::string xssclabel[NXSsc] = {"XSecScale=2.",
                                  "XSecScale=1.5",
                                  "XSecScale=1.25",
                                  "XSecScale=0.75",
                                  "XSecScale=0.5",
  ...
  BeamThinTargetConfig* btconf =
    (BeamThinTargetConfig*)hfile->Get("BertiniDefaultHARP/Beam_ThinTarget_Config");
  // btconf->Print();
  ...
  while (key) {
    ...
    if (htitle.find("Production of pi+") != std::string::npos)
      piplus_HARP.push_back(hexp);
    if (htitle.find("Production of pi-") != std::string::npos)
      piminus_HARP.push_back(hexp);
    key = (TKey*)next();
  }
  ...
  TPad* pad_xssc_harp_piminus_fw =
    new TPad("pad_xssc_harp_piminus_fw", "", 0.01, 0.25, 0.399, 0.97);
  TPad* pad_xssc_harp_piminus_la =
    new TPad("pad_xssc_harp_piminus_la", "", 0.401, 0.02, 0.99, 0.97);
  ...
  // NOTE: dir labels are "messed up"  as "gdq" (instead of "gqd that would stand for "gamma
  // quasi-deuteron")
  //
  // secondary pi+
  //
  TCanvas* cnv_gqdsc_harp_piplus = new TCanvas("cnv_gqdsc_harp_piplus", "", 1000, 700);
  TPad* pad_gqdsc_harp_piplus_fw =
    new TPad("pad_gqdsc_harp_piplus_fw", "", 0.01, 0.25, 0.399, 0.97);
  TPad* pad_gqdsc_harp_piplus_la =
    new TPad("pad_gqdsc_harp_piplus_la", "", 0.401, 0.02, 0.99, 0.97);
  ...
  TPad* pad_gqdsc_harp_piminus_fw =
    new TPad("pad_gqdsc_harp_piminus_fw", "", 0.01, 0.25, 0.399, 0.97);
  TPad* pad_gqdsc_harp_piminus_la =
    new TPad("pad_gqdsc_harp_piminus_la", "", 0.401, 0.02, 0.99, 0.97);
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    // if ( i == 3 ) VariantsHARP2Legend( NRsc, drsc, model, "HARP/piplus_RecordChi2",
    // rsclabel,colrsc, leg_rsc_harp_piplus );
    //
    ...
    // if ( i == 3 ) VariantsHARP2Legend( NRsc, drsc, model, "HARP/piminus_RecordChi2",
    // rsclabel,colrsc, leg_rsc_harp_piminus );
    //
    ...
    for (int j = 0; j < NRsc; ++j) {
      ...
      if (i == 3) {
        ...
        leg_rsc_harp_piplus->AddEntry(
          h1[j][0], Form("%s, chi2/NDF=%.2f", rsclabel[j].c_str(), chisq1[j][0] / ndf1[j][0]), "L");
        chisq1[j][1] = 0.;
        ndf1[j][1] = 0.;
        ...
        leg_rsc_harp_piminus->AddEntry(
          h1[j][1], Form("%s, chi2/NDF=%.2f", rsclabel[j].c_str(), chisq1[j][1] / ndf1[j][1]), "L");
      }
    }
    //
    // XSecScale variants
    ...
    for (int j = 0; j < NXSsc; ++j) {
      ...
      if (i == 3) {
        ...
        leg_xssc_harp_piplus->AddEntry(
          h2[j][0],
          Form("%s, chi2/NDF=%.2f", xssclabel[j].c_str(), chisq2[j][0] / ndf2[j][0]),
          "L");
        chisq2[j][1] = 0.;
        ndf2[j][1] = 0.;
        ...
        leg_xssc_harp_piminus->AddEntry(
          h2[j][1],
          Form("%s, chi2/NDF=%.2f", xssclabel[j].c_str(), chisq2[j][1] / ndf2[j][1]),
          "L");
      }
    }
    //
    ...
    for (int j = 0; j < NFsc; ++j) {
      ...
      if (i == 3) {
        ...
        leg_fsc_harp_piplus->AddEntry(
          h3[j][0], Form("%s, chi2/NDF=%.2f", fsclabel[j].c_str(), chisq3[j][0] / ndf3[j][0]), "L");
        chisq3[j][1] = 0.;
        ndf3[j][1] = 0.;
        ...
        leg_fsc_harp_piminus->AddEntry(
          h3[j][1], Form("%s, chi2/NDF=%.2f", fsclabel[j].c_str(), chisq3[j][1] / ndf3[j][1]), "L");
      }
    }
    //
    // GammaQDScale variants
    ...
    for (int j = 0; j < NGQDsc; ++j) {
      ...
      if (i == 3) {
        ...
        leg_gqdsc_harp_piplus->AddEntry(
          h4[j][0],
          Form("%s, chi2/NDF=%.2f", gqdsclabel[j].c_str(), chisq4[j][0] / ndf4[j][0]),
          "L");
        chisq4[j][1] = 0.;
        ndf4[j][1] = 0.;
        ...
        leg_gqdsc_harp_piminus->AddEntry(
          h4[j][1],
          Form("%s, chi2/NDF=%.2f", gqdsclabel[j].c_str(), chisq4[j][1] / ndf4[j][1]),
          "L");
      }
    }
    //
    ...
    for (int j = 0; j < NTrR; ++j) {
      ...
      if (i == 3) {
        ...
        leg_trr_harp_piplus->AddEntry(
          h5[j][0], Form("%s, chi2/NDF=%.2f", trrlabel[j].c_str(), chisq5[j][0] / ndf5[j][0]), "L");
        chisq5[j][1] = 0.;
        ndf5[j][1] = 0.;
        ...
        leg_trr_harp_piminus->AddEntry(
          h5[j][1], Form("%s, chi2/NDF=%.2f", trrlabel[j].c_str(), chisq5[j][1] / ndf5[j][1]), "L");
      }
    }
    ...
    if (i == 3) {
      ...
      leg_rsc_harp_piplus->AddEntry(
        h[0], Form("Default (RadSc=2.8), chi2/NDF=%.2f", (chisq[0] / ndf[0])), "L");
      leg_rsc_harp_piminus->AddEntry(
        h[1], Form("Default (RadSc=2.8), chi2/NDF=%.2f", (chisq[1] / ndf[1])), "L");
      leg_xssc_harp_piplus->AddEntry(
        h[0], Form("Default (XSecSc=1.), chi2/NDF=%.2f", (chisq[0] / ndf[0])), "L");
      leg_xssc_harp_piminus->AddEntry(
        h[1], Form("Default (XSecSc=1.), chi2/NDF=%.2f", (chisq[1] / ndf[1])), "L");
      leg_fsc_harp_piplus->AddEntry(
        h[0], Form("Default (FermiSc=0.685), chi2/NDF=%.2f", (chisq[0] / ndf[0])), "L");
      leg_fsc_harp_piminus->AddEntry(
        h[1], Form("Default (FermiSc=0.685), chi2/NDF=%.2f", (chisq[1] / ndf[1])), "L");
      leg_gqdsc_harp_piplus->AddEntry(
        h[0], Form("Default (GammaQDSc=1.), chi2/NDF=%.2f", (chisq[0] / ndf[0])), "L");
      leg_gqdsc_harp_piminus->AddEntry(
        h[1], Form("Default (GammaQDSc=1.), chi2/NDF=%.2f", (chisq[1] / ndf[1])), "L");
      leg_trr_harp_piplus->AddEntry(
        h[0], Form("Default (TrailingR=0.), chi2/NDF=%.2f", (chisq[0] / ndf[0])), "L");
      leg_trr_harp_piminus->AddEntry(
        h[1], Form("Default (TrailingR=0.), chi2/NDF=%.2f", (chisq[1] / ndf[1])), "L");
    }
  ...
  while (key1) {
    ...
    if (htitle1.find("Production of proton") != std::string::npos)
      proton_ITEP.push_back(hexp1);
    if (htitle1.find("Production of neutron") != std::string::npos)
      neutron_ITEP.push_back(hexp1);
    key1 = (TKey*)next1();
  }
  ...
  for (int i = 0; i < NPlotsITEP; ++i) {
    ...
    for (int j = 0; j < NXSsc; ++j) {
      ...
      if (i == 3) {
        ...
        leg_xssc_itep_neutron->AddEntry(
          h2[j][1],
          Form("%s, chi2/NDF=%.2f", xssclabel[j].c_str(), chisq2[j][1] / ndf2[j][1]),
          "L");
      }
    ...
    for (int j = 0; j < NGQDsc; ++j) {
      ...
      if (i == 3) {
        ...
        leg_gqdsc_itep_proton->AddEntry(
          h4[j][0],
          Form("%s, chi2/NDF=%.2f", gqdsclabel[j].c_str(), chisq4[j][0] / ndf4[j][0]),
          "L");
        chisq4[j][1] = 0.;
        ...
        leg_gqdsc_itep_neutron->AddEntry(
          h4[j][1],
          Form("%s, chi2/NDF=%.2f", gqdsclabel[j].c_str(), chisq4[j][1] / ndf4[j][1]),
          "L");
      }
...
TH1D*
GetHisto(std::string hname, double& max)
{
...
TH1D*
GetExpHARP(TH1D* hmc, const std::vector<TH1D*>& recs, double& max)
{
...
void
VariantsHARP2Legend(const int nv,
                    std::string* vdirs,
                    std::string model,
                    std::string rname,
                    std::string* vlabels,
                    ...
                    TLegend* leg)
{
  ...
  for (int j = 0; j < nv; ++j) {
    ...
    /* lent = */ leg->AddEntry(
      &marker, Form("%s, chi2/NDF=%.2f", vlabels[j].c_str(), chisq[j] / ndf[j]), "p");
    //      }
  }
