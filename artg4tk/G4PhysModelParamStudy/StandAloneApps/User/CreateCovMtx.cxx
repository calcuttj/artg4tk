...
main(int argc, char** argv)
{
  ...
  defaultchi2.insert(pair<string, vector<pair<string, pair<double, double>>>>(
    "Bertini", vector<pair<string, pair<double, double>>>()));
  map<string, vector<pair<string, pair<double, double>>>>::iterator idefchi2 =
    defaultchi2.find("Bertini");
  ...
  while (key) {
    ...
    if (objname.find("Default") != string::npos) {
      for (size_t ip = 0; ip < SecPartName.size(); ++ip) {
        ...
        (idefchi2->second)
          .push_back(
            make_pair(SecPartName[ip], make_pair(rchi2->GetSumChi2(), rchi2->GetSumNDF())));
      }
    ...
    if (!conf_exist) {
      config_vs_results.push_back(
        make_pair(TObjArray(), vector<pair<string, pair<double, double>>>()));
    ...
    for (size_t ip = 0; ip < SecPartName.size(); ++ip) {
      ...
      config_vs_results[icr].second.push_back(
        make_pair(SecPartName[ip], make_pair(rchi2->GetSumChi2(), rchi2->GetSumNDF())));
    }
  ...
  pdefaults.insert(
    pair<string, vector<pair<string, double>>>("Bertini", vector<pair<string, double>>()));
  ...
  variants.insert(
    pair<string, vector<pair<TH1D*, double>>>("Bertini", vector<pair<TH1D*, double>>()));
  ...
  for (int ih = 0; ih < nh; ++ih) {
    double chi2 = (imov->second)[ih].second;
    double wt = exp(-0.5 * (chi2 - minchi2));
    cout << " RadiusScale = " << (imov->second)[ih].first->GetBinContent(1) << "    chi2 = " << chi2
         << "    wt = " << wt << endl;
  }
  ...
  for (int ih = 0; ih < nh; ++ih) {
    double chi2 = (imov->second)[ih].second;
    double wt = exp(-0.5 * (chi2 - minchi2));
    cout << " XSecScale = " << (imov->second)[ih].first->GetBinContent(2) << "    chi2 = " << chi2
         << "    wt = " << wt << endl;
  }
  ...
  for (int ih = 0; ih < nh; ++ih) {
    double chi2 = (imov->second)[ih].second;
    double wt = exp(-0.5 * (chi2 - minchi2));
    cout << " FermiScale = " << (imov->second)[ih].first->GetBinContent(3) << "    chi2 = " << chi2
         << "    wt = " << wt << endl;
  }
  ...
  for (int ih = 0; ih < nh; ++ih) {
    double chi2 = (imov->second)[ih].second;
    double wt = exp(-0.5 * (chi2 - minchi2));
    cout << " TrailingRadius = " << (imov->second)[ih].first->GetBinContent(3)
         << "    chi2 = " << chi2 << "    wt = " << wt << endl;
  }
