...
MetaData::MetaData()
  : /* fBeamPID(0), fBeamMomentum(0.), */ fBeamLink(-1)
  , fTargetNucleus(0)
  , fSecondaryPID(0)
  , fObservable(-1)
  , fTitle("")
...
MetaData::operator==(const MetaData& rhs) const
{
  ...
  if (/* fBeamPID==rhs.fBeamPID && (fBeamMomentum-rhs.fBeamMomentum)<=1.e-10 && */
      fBeamLink == rhs.fBeamLink && fTargetNucleus == rhs.fTargetNucleus &&
      fSecondaryPID == rhs.fSecondaryPID && fObservable == rhs.fObservable) {
    return true;
  }
...
bool
MetaData::IsMatch(const int& beamid,
                  const double& mom,
                  const int& tgt,
                  const int& secid,
                  const int& observable) const
{

  if (IsBeamTargetMatch(beamid, mom, tgt) && secid == fSecondaryPID && observable == fObservable) {
    return true;
  }
...
bool
MetaData::IsBeamTargetMatch(const int& beamid, const double& mom, const int& tgt) const
{

  if (EmulateBeamLink(beamid, mom) == fBeamLink && tgt == fTargetNucleus) {
    return true;
  }
...
int
MetaData::EmulateBeamLink(const int& pid, const double& mom) const
{
  ...
  // --->   std::cout << " beam = " << pid << " momentum = " << mom << " beamlink = " << blnk <<
  // std::endl;
...
bool
JSON2Data::BuildDictionaries(const std::string& type, const std::string& jstr)
{

  if (type == "Particle")
    return BuildParticlesDict(jstr);
  if (type == "Material")
    return BuildTargetsDict(jstr);
  if (type == "Beam")
    return BuildBeamsDict(jstr);
  if (type == "Observable")
    return BuildObservablesDict(jstr);
  if (type == "Reference")
    return BuildReferencesDict(jstr);
  if (type == "Datatypes")
    return BuildDataTypesDict(jstr);

  return false;
}

void
JSON2Data::ParseMetaData(const std::string& jstr)
{
...
bool
JSON2Data::BuildParticlesDict(const std::string& jstr)
{
...
bool
JSON2Data::BuildTargetsDict(const std::string& jstr)
{
...
bool
JSON2Data::BuildBeamsDict(const std::string& jstr)
{
...
bool
JSON2Data::BuildObservablesDict(const std::string& jstr)
{
...
bool
JSON2Data::BuildReferencesDict(const std::string& jstr)
{
...
bool
JSON2Data::BuildDataTypesDict(const std::string& jstr)
{
...
TH1D*
JSON2Data::Convert2Histo(const std::string& jstr, const char* hname)
{
  ...
  if (dtype >= 1000) {
    ...
    // At least, we'll go like this until we get a better idea and/or time to refine the
    // infrastructure
    //
    mf::LogInfo log;
...
TGraphErrors*
JSON2Data::Convert2Graph(const std::string& jstr, const char* grname)
{
...
void
JSON2Data::ConvertGraph2Histo(const char* hname)
{
