...
//
G4_DECLARE_PHYSCONSTR_FACTORY(G4HadronElasticPhysicsBias);
//

G4HadronElasticPhysicsBias::G4HadronElasticPhysicsBias(G4int ver, const G4String& nam)
  : G4VPhysicsConstructor(nam), verbose(ver)
{
  if(verbose > 1) { 
    G4cout << "### G4HadronElasticPhysicsBias: " << GetPhysicsName() 
	   << G4endl; 
  }
}

G4HadronElasticPhysicsBias::~G4HadronElasticPhysicsBias()
{}

void G4HadronElasticPhysicsBias::ConstructParticle()
{
  // G4cout << "G4HadronElasticPhysicsBias::ConstructParticle" << G4endl;
...
void G4HadronElasticPhysicsBias::ConstructProcess()
{
  const G4double elimitAntiNuc = 100.*MeV;
  const G4double delta = 0.1*MeV;
  if(verbose > 1) {
    G4cout << "### HadronElasticPhysicsBias::ConstructProcess: "
  ...
  while( (*myParticleIterator)() )
  {
    ...
    if(pname == "anti_lambda"  ||
      ...
      if(verbose > 1) {
	G4cout << "### HadronElasticPhysicsBias: " << hel->GetProcessName()
	...
	G4cout << "### HadronElasticPhysicsBias: " << hel->GetProcessName()
      ...
      std::cout << "Setting Proton Elastic: " << fProtonBias << std::endl;
      hel->MultiplyCrossSectionBy(fProtonBias);
	...
	G4cout << "### HadronElasticPhysicsBias: " << hel->GetProcessName()
      ...
      std::cout << "Setting Neutron Elastic: " << fNeutronBias << std::endl;
      hel->MultiplyCrossSectionBy(fNeutronBias);
	...
	G4cout << "### HadronElasticPhysicsBias: " << hel->GetProcessName()
      ...
      if (pname == "pi+") {
        std::cout << "Setting PiPlus Elastic: " << fPiPlusBias << std::endl;
        hel->MultiplyCrossSectionBy(fPiPlusBias);
      }
      else if (pname == "pi-") {
        std::cout << "Setting PiMinus Elastic: " << fPiMinusBias << std::endl;
        hel->MultiplyCrossSectionBy(fPiMinusBias);
      }
...
G4HadronicProcess* 
G4HadronElasticPhysicsBias::GetElasticProcess(const G4ParticleDefinition* part) const
{
...
G4HadronElastic* 
G4HadronElasticPhysicsBias::GetElasticModel(const G4ParticleDefinition* part) const
{
...
G4HadronicProcess* G4HadronElasticPhysicsBias::GetNeutronProcess() const
{
  return GetElasticProcess(G4Neutron::Neutron());
}

G4HadronElastic* G4HadronElasticPhysicsBias::GetNeutronModel() const
{
  return GetElasticModel(G4Neutron::Neutron());
}

void G4HadronElasticPhysicsBias::AddXSection(const G4ParticleDefinition* part,
