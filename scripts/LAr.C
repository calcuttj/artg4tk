...
// root [6] rindextable();                   // refraction index between 110 and 700 nm in
// nsteps=100 for T=83.81 K root [7] rindextable(200,300,10,3);       // refraction index between
// 200 and 300 nm in nsteps=10 for T=90 K root [8] rayleightable();                 // rayleigh
// length between 110 and 700 nm in nsteps=100 for T=83.81 K root [9] rayleightable(200,300,10,3);
// // rayleigh length between 200 and 300 nm in nsteps=10 for T=90 K
//------------------------------------------------------------------------------------------------------------------
#include "TAxis.h"
#include "TCanvas.h"
#include "TF1.h"
...
const double alngth =
  66.; // LAr attenuation length in cm at 128 nm
       // from N. Ishida et al :
...
// A. C. Sinnock, B. L. Smith, Refractive indices of the condensed inert gases, Phys. Rev. 181
// (1969) 1297-1307.
const Int_t nr = 9;
const Double_t Lambda[nr] = {361.2, 365.0, 406.3, 435.8, 475.3, 508.6, 546.1, 578.0, 643.9};
const Double_t R[nr] = {1.2395, 1.2392, 1.2372, 1.2361, 1.2349, 1.2341, 1.2334, 1.2328, 1.2321};
// Liquid Argon
...
double
sellmeier_LAr(double* x, double* p)
{
  double la0 = p[0];
  double laUV = p[1];
  double laIR = p[2];
  double lambda = x[0] * 1e-7; // convert from nm to cm
  double nsquare = la0 + (laUV * lambda * lambda) / (lambda * lambda - lambdaUV * lambdaUV) +
                   (laIR * lambda * lambda) / (lambda * lambda - lambdaIR * lambdaIR);
  return sqrt(nsquare);
}
double
sellmeier_LAr(double lambda, int index)
{
  lambda = lambda * 1e-7;
...
double
sellmeierpe_LAr(double* x, double* p)
{
  double la0 = p[0];
  double laUV = p[1];
  double laIR = p[2];
...
double
sellmeierpe_LAr(double x, int index)
{
...
// emin and emax in nm
void
sellmeierLAr(double emin = 110, double emax = 700)
{
...
double
lrayleigh(Double_t* x, Double_t* p)
{
  double lambda = x[0];
  int index = int(p[0]);
  ...
  double br = kbTrhokT[index] * (((n * n - 1) * (n * n + 2)) / (3. * density[index]) *
                                 (((n * n - 1) * (n * n + 2)) / (3. * density[index])));
  l = l * br;
  l = 1. / l;
  return l;
}
double
lrayleigh(double lambda, int index)
{
  double n = sellmeier_LAr(lambda, index);
  lambda = lambda * 1.e-7; // convert from nm to cm
  double l = p3times16 / (6. * lambda * lambda * lambda * lambda);
...
void
rayleigh(double emin = 110, double emax = 400)
{
  const double minlambda = 110;
...
//----------------------------------------------------------------------
void
rayleightable(double emin = 110, double emax = 700, int nsteps = 100, int index = 0)
{
  const double minlambda = 110;
  const double maxlambda = 700;
  if (emin < minlambda || emax > maxlambda) {
  ...
  cout << "     \<matrix name=\"RAYLEIGH\" coldim=\"2\" values=\"" << photone << "*eV " << n
       << "*cm" << endl;
...
// --------------------------------------------------------------------
void
rindextable(double emin = 110, double emax = 700, int nsteps = 100, int index = 0)
{
  const double minlambda = 110;
  const double maxlambda = 700;
  if (emin < minlambda || emax > maxlambda) {
